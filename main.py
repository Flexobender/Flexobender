...
class Order(object):
    ...
    def __init__(self, name):
        ...
        self.machine.add_transition(
            trigger="size", source="big_or_little", dest="thin_or_thick"
        )
        self.machine.add_transition(
            trigger="thickness", source="thin_or_thick", dest="payment"
        )
        self.machine.add_transition(trigger="pay", source="payment", dest="checking")
        self.machine.add_transition(
            trigger="confirm", source="checking", dest="big_or_little"
        )
...
def get_text_messages(message):
    ...
    if message.text in ("Начнем", "/start"):
        bot.send_message(
            message.from_user.id, "Какую вы хотите пиццу? Большую или маленькую?"
        )
        ...
        order.size()
        bot.send_message(
            message.from_user.id, "На каком тесте Вам сделать? На толстом, На тонком"
        )
        ...
        order.thickness()
        bot.send_message(
            message.from_user.id, "Как вы будете платить? Наличкой, Картой"
        )
        ...
        bot.send_message(
            ...
            "Вы хотите {x} пиццу, {z} тесте, оплата - {y}?".format(
                x=str.lower(pizza_size),
                y=str.lower(type_of_payment),
                z=str.lower(pizza_thickness),
            ),
        )
        order.pay()
        ...
        bot.send_message(
            message.from_user.id,
            "Давайте проверим. Какую вы хотите пиццу? Большую или Маленькую? ",
        )
        order.confirm()
        ...
        bot.send_message(
            message.from_user.id,
            'Я тебя не понимаю. Напиши "Помощь" или исправь опечатку.',
        )
